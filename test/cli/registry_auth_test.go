...
func TestRegistryAuth(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			args: []string{"-vv", "registry:localhost:5000/something:latest"},
			env: map[string]string{
				"GRIFFON_REGISTRY_AUTH_AUTHORITY": "localhost:5000",
				"GRIFFON_REGISTRY_AUTH_USERNAME":  "username",
				"GRIFFON_REGISTRY_AUTH_PASSWORD":  "password",
			},
			assertions: []traitAssertion{
				assertInOutput("source=OciRegistry"),
				assertInOutput("localhost:5000/something:latest"),
		...
		},
		{
			...
			env: map[string]string{
				"GRIFFON_REGISTRY_AUTH_AUTHORITY": "localhost:5000",
				"GRIFFON_REGISTRY_AUTH_TOKEN":     "token",
			},
			assertions: []traitAssertion{
				assertInOutput("source=OciRegistry"),
				assertInOutput("localhost:5000/something:latest"),
		...
		},
		{
			...
			args: []string{"-vv", "registry:localhost:5000/something:latest"},
			env: map[string]string{
				"GRIFFON_REGISTRY_INSECURE_USE_HTTP": "true",
			},
			assertions: []traitAssertion{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			cmd, stdout, stderr := runGriffon(t, test.env, test.args...)
