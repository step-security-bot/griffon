...
func getGriffonCommand(tb testing.TB, args ...string) *exec.Cmd {
	tb.Helper()

	return exec.Command(
		getGriffonSnapshotLocation(tb, runtime.GOOS),
		append(
...
func getGriffonSnapshotLocation(tb testing.TB, goOS string) string {
	if os.Getenv("GRIFFON_BINARY_LOCATION") != "" {
		// GRIFFON_BINARY_LOCATION is the absolute path to the snapshot binary
		return os.Getenv("GRIFFON_BINARY_LOCATION")
	}
...
func runGriffon(tb testing.TB, env map[string]string, args ...string) (*exec.Cmd, string, string) {
	tb.Helper()

	cmd := getGriffonCommand(tb, args...)
	if env == nil {
		env = make(map[string]string)
	}

	// we should not have tests reaching out for app update checks
	env["GRIFFON_CHECK_FOR_APP_UPDATE"] = "false"
