...
the previous Griffon version as well as the local version).
...
We'll need to use the UUIDs to explore the labels, so copy the first UUID, which we can see was run against the last Griffon release (`griffon@v0.53.1`). Use the UUID to explore and edit the results with
...
to the Griffon pull request and push as part of the in-progress PR
against Griffon. The PR will now use the updated match labels when running
the quality check.
