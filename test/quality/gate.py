...
class Gate:
    ...
    def __post_init__(
            self, label_comparisons: Optional[list[comparison.AgainstLabels]],
...
def validate(cfg: config.Application,
             result_set: str,
             images: list[str],
    ...
    for image, result_states in result_set_obj.result_state_by_image.items():
        ...
        gate = validate_image(
            cfg, [s.config.path for s in result_states],
...
def validate_image(cfg: config.Application,
    ...
    relative_comparison = yardstick.compare_results(
        descriptions=descriptions, year_max_limit=cfg.default_max_year)
